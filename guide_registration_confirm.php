...
if(empty($post['loginid'])){
  ...
  if($rg->loginidNGCheck($post['loginid'], 't_guide')){
    $er_loginid = "ログインIDがすでに使用されています。";
    $_SESSION['er_loginid'] = $er_loginid;
    $erCount ++;
  }
}
if(empty($post['name'])){
...
$view -> setAssign("er_image",$er_image);
$view -> setAssign("self_introduction",$self_introduction);

$view -> screenView('guide_registration_confirm');
